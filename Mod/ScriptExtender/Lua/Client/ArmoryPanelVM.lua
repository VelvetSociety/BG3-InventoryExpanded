...
local function GetDoubleClickMs()
    local ok, v = pcall(MCM.Get, "double_click_ms")
    return (ok and v) or DOUBLE_CLICK_MS
end
...
local armoryFilterState = {
    ...
    armorTypes  = {},   -- { Clothing=true, Light=true, ... }
    equipMode   = "Both",  -- "Both", "Equipped", or "Unequipped"
    sortBy      = nil,
    sortAscending = true,
}

-- Map raw BG3 ArmorType stat values → display category
...
-- Slots that can hold clothing (ArmorType=None items in these slots = Clothing)
local ARMOR_SLOTS = { Breast=true, Helmet=true, Boots=true, Gloves=true, Cloak=true }

-- Returns "Clothing"/"Light"/"Medium"/"Heavy", or nil for accessories/non-armor.
local function GetArmorCategory(armorType, slot)
    if armorType and armorType ~= "" and armorType ~= "None" then
        return ARMOR_CATEGORY[armorType]  -- explicit type (Cloth, Leather, ScaleMail, etc.)
    end
    -- ArmorType=None: only treat as Clothing if it's in a body-armor slot
    if ARMOR_SLOTS[slot] then return "Clothing" end
    return nil  -- ring, amulet, shield, underwear, etc. — not an armor type
end
...
local function CountArmoryFilters()
    ...
    for _ in pairs(armoryFilterState.armorTypes) do n = n + 1 end
    if armoryFilterState.equipMode and armoryFilterState.equipMode ~= "Both" then n = n + 1 end
    return n
end
...
local function UpdateFilterSectionVisibility()
    pcall(function()
        ...
        local showArmorType = (cat == "Armor" or cat == "All") and "True" or "False"
        pcall(function() vm.FilterSection_DamageDice = showDice end)
        pcall(function() vm.FilterSection_DamageType = showDmgType end)
        pcall(function() vm.FilterSection_ArmorType = showArmorType end)
    end)
end
...
local function ClearIrrelevantFilters()
    ...
    if cat ~= "Armor" and cat ~= "All" then
        for k in pairs(armoryFilterState.armorTypes) do armoryFilterState.armorTypes[k] = nil end
    end
end
...
local function SyncAllFilterProps()
    pcall(function()
        ...
        -- Armor types
        for _, at in ipairs({"Clothing", "Light", "Medium", "Heavy"}) do
            pcall(function() vm["FilterArmorType_" .. at] = armoryFilterState.armorTypes[at] and "True" or "False" end)
        end

        -- Equipped mode (radio)
        local em = armoryFilterState.equipMode or "Both"
        pcall(function()
            vm.FilterEquipMode_Both       = em == "Both"       and "True" or "False"
            vm.FilterEquipMode_Equipped   = em == "Equipped"   and "True" or "False"
            vm.FilterEquipMode_Unequipped = em == "Unequipped" and "True" or "False"
        end)
    end)
end
...
local function RegisterTypes()
    ...
    local vmProps = {
        ...
        FilterSection_ArmorType  = { Type = "String", Notify = true },
        ...
        -- AC range filters
        FilterArmorType_Clothing = { Type = "String", Notify = true },
        FilterArmorType_Light    = { Type = "String", Notify = true },
        FilterArmorType_Medium   = { Type = "String", Notify = true },
        FilterArmorType_Heavy    = { Type = "String", Notify = true },
        ToggleArmorType_Clothing = { Type = "Command" },
        ToggleArmorType_Light    = { Type = "Command" },
        ToggleArmorType_Medium   = { Type = "Command" },
        ToggleArmorType_Heavy    = { Type = "Command" },
    ...
    isRegistered = true
    _P("[InventoryExpanded] ArmoryPanelVM types registered")
end
...
local function CollectNativeSlots()
    ...
    pcall(function()
        ...
        -- Also collect from camp chests
        pcall(function()
            local campChests = cp.UIData.CampChests
            if not campChests then return end
            for i = 1, #campChests do
                pcall(function()
...
function ArmoryPanelVM.PopulateEquippedSlots()
    ...
    _P("[InventoryExpanded] Armory equipped slots populated for " .. charName
...
function ArmoryPanelVM.PopulateFilteredItems()
    ...
    local hasArmorTypes = next(armoryFilterState.armorTypes)

    if hasDice or hasDmgTypes or hasArmorTypes then
        local passed = {}
        for _, item in ipairs(filtered) do
            ...
            if keep and hasArmorTypes then
                local cat = GetArmorCategory(item.ArmorType, item.Slot)
                if not cat or not armoryFilterState.armorTypes[cat] then keep = false end
            end
    ...
    -- Step 3.5: Equipped mode filter
    if armoryFilterState.equipMode and armoryFilterState.equipMode ~= "Both" then
        filtered = FilterEngine.FilterMulti(filtered, { equipped = armoryFilterState.equipMode })
    end
    ...
    for _, entry in ipairs(passingEntries) do
        ...
        -- Icon/rarity/stack rendered by Template.Item natively; only equipped badge needs DataStore.
    ...
    _P("[InventoryExpanded] Armory filtered: " .. count .. " " .. selectedSlotLabel .. " items")
end

--- Full refresh: both sides.
-- Throttled: the native UI tree walk inside PopulateFilteredItems is unsafe to run
-- concurrently with panel show/hide transitions. Calls that arrive while a refresh
-- is in-flight are coalesced into one follow-up execution after the cooldown.
local _refreshBusy    = false
local _refreshPending = false
local doRefreshNow    -- forward-declared for self-reference in timer callback
...
function ArmoryPanelVM.TryBind()
    ...
    pcall(function()
        freshVM.RefreshCommand:SetHandler(function()
            ...
            -- Calling Refresh() here too causes a double rebuild race that crashes Noesis.
            Mods.InventoryExpanded.RequestRefresh()
        end)
    end)
    ...
    pcall(function()
        freshVM.ClearAllFiltersCommand:SetHandler(function()
            ...
            for k in pairs(armoryFilterState.damageTypes) do armoryFilterState.damageTypes[k] = nil end
            for k in pairs(armoryFilterState.armorTypes) do armoryFilterState.armorTypes[k] = nil end
            armoryFilterState.equipMode = "Both"
            armoryFilterState.sortBy = nil
            armoryFilterState.sortAscending = true
            SyncAllFilterProps()
            UpdateArmorySortIndicators()
    ...
    -- Wire armor type toggle commands
    local armorTypeFilters = {
        {cmd="ToggleArmorType_Clothing", prop="FilterArmorType_Clothing", key="Clothing"},
        {cmd="ToggleArmorType_Light",    prop="FilterArmorType_Light",    key="Light"},
        {cmd="ToggleArmorType_Medium",   prop="FilterArmorType_Medium",   key="Medium"},
        {cmd="ToggleArmorType_Heavy",    prop="FilterArmorType_Heavy",    key="Heavy"},
    }
    for _, f in ipairs(armorTypeFilters) do
        pcall(function() freshVM[f.cmd]:SetHandler(MakeArmoryToggleHandler(f.prop, armoryFilterState.armorTypes, f.key)) end)
    end

    -- Wire equip mode radio commands
    local function MakeEquipModeHandler(mode)
        return function()
            armoryFilterState.equipMode = mode
            SyncAllFilterProps()
            UpdateArmoryFilterCount()
            ArmoryPanelVM.PopulateFilteredItems()
        end
    ...
    pcall(function()
        freshVM.EquipSelectedCommand:SetHandler(function()
            ...
            if idx ~= _lastEquipIdx or (now - _lastEquipTime) > GetDoubleClickMs() then
                _lastEquipIdx  = idx
                _lastEquipTime = now
    ...
    freshVM.FilterSection_ArmorType = "True"

    -- Initialize equip mode props (default: Both is active)
    pcall(function()
    ...
    local allFilterProps = {
        ...
        "FilterArmorType_Clothing", "FilterArmorType_Light", "FilterArmorType_Medium", "FilterArmorType_Heavy",
        ...
        local function pollCharChange()
            pcall(function()
                ...
                if newCharPtr and _lastCharPtr and newCharPtr ~= _lastCharPtr then
                    ...
                    _P("[InventoryExpanded] Armory: character changed, refreshing in 300ms")
                    pcall(function()
...
function ArmoryPanelVM.Init()
    RegisterTypes()
    _P("[InventoryExpanded] ArmoryPanelVM initialized, waiting for widget...")
...
function ArmoryPanelVM.ForceHide()
...
function ArmoryPanelVM.Toggle()
    ...
    if not vm then
        _P("[InventoryExpanded] Armory Toggle: cannot get VM")
        return
    end

    local selectedChar, charUUID = getSelectedCharInfo()
    if not selectedChar or not charUUID then
        local widget = FindWidget()
        ForceClosed(widget, vm)
        vm.StatusText = "No character selected"
        _P("[InventoryExpanded] Armory Toggle: no selected character context")
        return
    end
    ...
    if vm.PanelVisible then
        ...
        -- Do NOT call Refresh() here — calling it alongside RequestRefresh() creates
        -- a double rebuild race that crashes Noesis (see RefreshCommand handler).
...
Mods.InventoryExpanded.ArmoryPanelVM = ArmoryPanelVM
