...
function FilterEngine.FilterMulti(items, filters)
    ...
    local equipMode = filters.equipped  -- "Equipped", "Unequipped", or nil/"Both"
    ...
    for _, item in ipairs(items) do
        ...
        if pass and equipMode and equipMode ~= "Both" then
            local wantEquipped = (equipMode == "Equipped")
            if (item.Equipped == true) ~= wantEquipped then pass = false end
        end
...
if Ext then
    ...
    Mods.InventoryExpanded.FilterEngine = FilterEngine
end
