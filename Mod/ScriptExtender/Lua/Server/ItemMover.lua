...
--- Get the camp chest UUID for a character, or nil if not found.
-- Uses DB_Camp_UserCampChest which is the same source as InventoryCollector.
local function GetCampChestUUID(charUUID)
    local chestUUID = nil
    pcall(function()
        local userId = Osi.GetReservedUserID(charUUID)
        if not userId then return end
        local rows = Osi.DB_Camp_UserCampChest:Get(userId, nil)
...
function ItemMover.EquipItem(itemUUID, targetCharUUID, targetSlot)
    ...
    -- Helper: move a slot's occupant to the camp chest, falling back to character inventory.
    local function vacateSlot(osiSlotName)
        local uid = getSlotOccupant(osiSlotName)
        if uid and uid ~= itemUUID then
            _P("[InventoryExpanded] Vacating " .. osiSlotName .. ": " .. tostring(uid))
            local chestUUID = GetCampChestUUID(targetCharUUID)
            if chestUUID then
                local ok, err = pcall(function() Osi.ToInventory(uid, chestUUID, 1, 0, 0) end)
                _P("[InventoryExpanded] Vacate → camp chest: ok=" .. tostring(ok) .. " err=" .. tostring(err))
                if not ok then
                    pcall(function() Osi.ToInventory(uid, targetCharUUID, 1, 0, 0) end)
                end
            else
                _P("[InventoryExpanded] No camp chest found, vacating to character inventory")
                pcall(function() Osi.ToInventory(uid, targetCharUUID, 1, 0, 0) end)
            end
        end
        return uid
    ...
    if targetSlot == "MeleeOffHand" or targetSlot == "RangedOffHand" or targetSlot == "Ring2" then
        ...
        if primaryUUID and primaryUUID ~= itemUUID then
            ...
            _P("[InventoryExpanded] Temporarily removing " .. primarySlot .. ": " .. tostring(primaryUUID))
            pcall(function() Osi.ToInventory(primaryUUID, targetCharUUID, 1, 0, 0) end)

            -- Re-equip primary weapon → goes to empty MainHand/Ring
            _P("[InventoryExpanded] Re-equipping primary to " .. primarySlot)
            ...
            _P("[InventoryExpanded] Equipping new item with " .. primarySlot .. " occupied")
...
function ItemMover.SendToCamp(itemUUID)
    ...
    -- Find the owner's camp chest via DB_Camp_UserCampChest
    local charUUID = nil
    pcall(function() charUUID = Osi.GetOwner(itemUUID) end)

    local chestUUID = charUUID and GetCampChestUUID(charUUID)
    if chestUUID then
        local ok, err = pcall(function() Osi.ToInventory(itemUUID, chestUUID, 1, 0, 0) end)
        if ok then
    ...
    return false, "Send to camp failed: could not find camp chest for item owner"
end

-- Export
Mods = Mods or {}
Mods.InventoryExpanded = Mods.InventoryExpanded or {}
Mods.InventoryExpanded.ItemMover = ItemMover
